...
// A branded pulsing dot instead of Leaflet's default pin — matches
// BrandMark's own "live" cue (same .signal-pulse-ring animation) and
// sidesteps Leaflet's well-known bundler issue where its default marker
// icon images resolve to the wrong path under webpack/Turbopack. A
// DivIcon needs no image assets at all, so that bug never applies here.
...
export default function LocationMapInner() {
  ...
  return (
    ...
    >
      ...
      />
      {/* #1d4ed8 = --brand-cobalt, hardcoded rather than var(...) -- Leaflet
          sets SVG path color via setAttribute, which (unlike inline style)

...
export async function createObservation(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      author_id: authorId,
      // input.occurredAt comes from a <input type="datetime-local"> with no
      // timezone in its value — parsed as Astana wall-clock time (see
      // lib/timezone.ts), not the server's own (UTC) local time.
